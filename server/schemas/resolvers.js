...
const resolvers = {
  Query: {
    ...
    quizSet: async (parent, { quizSetId }, context) => {
      // if (context.user) {
      const quizSet = await QuizSet.findById({ _id: quizSetId });
      return quizSet;
      // }
    },
    therapyNote: async (parent, { therapyNoteId }, context) => {
      if (context.user) {
        const therapyNote = await TherapyNote.findById({ _id: therapyNoteId });
        return therapyNote;
      }
    },
  },
  Mutation: {
    ...
    addQuizResult: async (
      parent,
      { quizSetId, quizTaken, quizAnswer },
      context
    ) => {
      // if (context.user) {
